...
- Each matchday is read through one projection, and unpinned rounds resolve to the newest batch that actually *forecast* them — not the newest batch, which for a played round kept no pick. The season, its distributions and the public export are all composed from those attachments (`matchday_projections`, `resolveMatchdayProjections`).
